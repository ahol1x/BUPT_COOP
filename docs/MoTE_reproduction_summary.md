...
| 3 | ImageNet-A B0-Inc20, five seeds, mean/std | Pending |
| 4 | ImageNet-R B0-Inc20, five seeds, mean/std | Pending |
...
1. Reproduce MoTE on ImageNet-A B0-Inc20.
2. Reproduce MoTE on ImageNet-R B0-Inc20.
3. Use seeds 1991, 1992, 1993, 1994, and 1995 for both datasets.
4. Run seed 1993 as a smoke test before launching the full five-seed jobs.
5. Save configs, per-seed metrics, mean/std, Top-1 and Top-5 curves, logs, runtime, trainable parameters, total parameters, adapter count, and memory statistics.
6. After all five datasets are complete, execute checklist items 6-13.
7. Only then continue the paper chain: LDEPrompt, QKD, and TaE.
8. After all four papers are reproduced, compare their ViT modifications, frozen and trainable parameters, inserted modules, parameter expansion, and inference logic to design the adaptive model-selection algorithm.
