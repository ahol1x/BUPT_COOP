...
# 1. Fix timm import in backbone/linears.py
...
# 2. Make missing CodaPrompt optional in utils/inc_net.py
...
# Fix possible typo in mote_limit branch if present
s = s.replace("vit_moe_limit.", "vit_mote_limit.")
p.write_text(s)
print("patched utils/inc_net.py")

# 3. Patch missing attributes and eval output unpacking in models/mote.py
p = Path("models/mote.py")
s = p.read_text()

# Add missing attributes after self.args assignment if not already assigned
